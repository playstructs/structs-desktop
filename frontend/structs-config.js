...
if (window.__STRUCTS_CONFIG__ && window.__TAURI__) {
  ...
  // ── Silent-Audio Keepalive (macOS WKWebView) ──
  // When the window is occluded, WKWebView throttles/suspends the whole
  // WebContent process. `background_throttling: Disabled` only stops timer
  // coalescing, not process suspension, so the grass/NATS websocket loop, its
  // listeners, and JS timers stall. WebKit exempts a page from process
  // throttling while it holds a "playing audio" assertion — so we keep a
  // zero-gain AudioContext running. Inaudible, and (unlike a MediaSession
  // <audio> element) it stays out of macOS "Now Playing".
  (function setupAudioKeepalive() {
    try {
      // Gate to macOS — the throttling behavior is WKWebView-specific.
      if (!/Macintosh/.test(navigator.userAgent)) return;

      var AudioCtx = window.AudioContext || window.webkitAudioContext;
      if (!AudioCtx) {
        console.warn('[Structs Keepalive] AudioContext unavailable; skipping');
        return;
      }
      ...
      // Try immediately; if blocked by autoplay policy, resume on first gesture.
      resume();
      if (ctx.state === 'suspended') {
        var gestures = ['click', 'keydown', 'touchstart', 'pointerdown'];
        var onGesture = function() {
          resume();
          gestures.forEach(function(g) {
            window.removeEventListener(g, onGesture, true);
          });
        };
        gestures.forEach(function(g) {
          window.addEventListener(g, onGesture, true);
        });
      }

      // Watchdog: keep the context running, and re-resume on foreground.
      setInterval(resume, 10000);
      window.addEventListener('structs:grass-resume-check', resume);

      console.info('[Structs Keepalive] Silent-audio keepalive active (state=' + ctx.state + ')');

      // ── TEMP DIAGNOSTIC — remove after verifying the fix ──
      // This is itself a throttled JS timer. While occluded: steady ~1000ms
      // gaps = fix working; gaps ballooning to many seconds = still throttled.
      ...
      setInterval(function() {
        ...
        _lastTick = now;
        if (gap > 1500) {
          console.warn('[Structs Keepalive] tick gap = ' + gap + 'ms (throttled?) audio=' + ctx.state);
        } else {
          console.info('[Structs Keepalive] tick gap = ' + gap + 'ms audio=' + ctx.state);
        }
      }, 1000);
    } catch (e) {
      console.warn('[Structs Keepalive] setup failed:', e);
    }
  })();
